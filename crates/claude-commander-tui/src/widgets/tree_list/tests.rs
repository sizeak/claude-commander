use super::*;
use claude_commander_core::git::PrState;
use claude_commander_core::session::{ProjectId, SessionId, SessionStatus};
...
fn server_header_renders_version_mismatch_annotation() {
    use claude_commander_core::backend::{BackendId, ConnectionState, VersionMismatch};
...
fn worktree_shows_keep_alive_marker() {
    ...
    assert!(
        !plain.contains(crate::widgets::status_glyph::KEEP_ALIVE_MARKER),
    ...
    assert!(
        marked.contains(crate::widgets::status_glyph::KEEP_ALIVE_MARKER),
...
fn worktree_shows_pending_comment_marker() {
    ...
    assert!(
        !plain.contains(crate::widgets::status_glyph::COMMENT_MARKER),
        "unexpected marker:\n{plain}"
    );
    ...
    assert!(
        marked.contains(crate::widgets::status_glyph::COMMENT_MARKER),
...
fn test_pr_badge_color_open() {
    let theme = Theme::basic();
    let c = crate::widgets::pr_colors::pr_badge_color(
        &theme,
        Some(PrState::Open),
        false,
        false,
...
fn test_pr_badge_color_unknown_state_uses_pr_merged_flag_for_merged() {
    ...
    let theme = Theme::basic();
    let c =
        crate::widgets::pr_colors::pr_badge_color(&theme, None, true, false, &[], &review_labels());
    assert_eq!(c, theme.status_pr_merged);
}
