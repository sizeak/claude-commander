...
use crate::list_nav::{DOUBLE_CLICK_WINDOW, list_index_at, wheel_step};
use claude_commander_core::config::AppState;
use claude_commander_core::session::{SessionStatus, display_branch};
...
pub fn run_session_picker(out_path: &Path, current: Option<&str>) -> Result<()> {
    ...
    let state = AppState::load().map_err(TuiError::Core)?;
...
mod tests {
    use super::*;
    use chrono::Duration;
    use claude_commander_core::session::{Project, ProjectId, SessionId, WorktreeSession};
