...
- The clap definitions (`crates/claude-commander/src/cli_args.rs`) are the exception that stays in the binary crate, and must not be moved into core: a clap derive takes its program name and version from the package it compiles in, so in core `--version` printed `claude-commander-core` and the generated commander CLI reference documented unrunnable `claude-commander-core <sub>` commands. Library code that needs the command tree is handed it (`App::new`, `commander::ensure_session` take a `&clap::Command`) rather than building its own. Same rule for any other binary identity: `NAME`/`VERSION` in `main.rs` are the binary's; `claude_commander_core::VERSION` is the library's (telemetry `lib_version`, server-build comparison) and is never user-facing.
