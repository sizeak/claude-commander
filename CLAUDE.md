...
- **`annotation/`** — local diff-review annotations: `Annotation` model + persisted `AnnotationStore`, snippet-based `reanchor` (drift detection), markdown composition, vim visual-mode selection math, and apply-delivery decision (`decide_send`/`ApplyOutcome`).
