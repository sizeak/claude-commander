...
- **A shared scorer has to be a *function*.** `fuzzy-matcher`'s `SkimMatcherV2` keeps per-thread scratch buffers (`src/skim.rs:610`) grown with `Vec::resize` (`src/skim.rs:394`), which initialises only new cells — so a matcher reused across calls returns history-dependent scores. Caching one in a `static` made `"list-session"` score 142 alone but 145 after a longer haystack, turning a strict ordering into a tie; two frontends on different threads would then rank the same list differently, defeating the point of sharing. `viewmodel::query` builds a fresh matcher per call (measured cost: ~27µs on a 180-field keystroke) and `scoring_is_independent_of_call_history` guards it. Do **not** reach for `use_cache(false)`: in 0.3.7 it panics (`src/skim.rs:929-933` replaces `RefCell`s whose borrows are live).
