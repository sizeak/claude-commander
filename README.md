...
| `C` | Open the commander session — opens a status modal; Enter attaches (a persistent, project-less Claude session that coordinates others; requires `commander_enabled = true`) |
...
# CLI. Disabled by default. `C` opens a status modal showing the commander's
# live running/agent state (Enter attaches, Esc closes); while it is running a
# `● Commander` chip appears in the footer status bar.
