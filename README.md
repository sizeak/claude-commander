...
| `Alt-c` | Open/close the conversation overlay: a full-screen chat with a dedicated Claude session whose replies stream in and are spoken aloud via an OpenAI-compatible TTS engine (see [Conversation mode](docs/configuration.md#conversation-mode-tts)). The session keeps running when the overlay is closed |
