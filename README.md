...
| `C` | Open the commander session (a persistent, project-less Claude session that coordinates others; requires `commander_enabled = true`). While enabled, a status panel under the preview shows the commander's live running/agent state. |
...
# CLI. Disabled by default. When enabled, a status panel under the preview pane
# shows the commander's live running/agent state.
