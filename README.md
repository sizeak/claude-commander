...
| `Alt-v` | Toggle conversation mode: speak the commander's replies aloud via an OpenAI-compatible TTS engine (requires `commander_enabled` and `[conversation]` config; see [Conversation mode](docs/configuration.md#conversation-mode-tts)). A `🔊 TTS` chip shows in the footer while active |
