{
  ...
  outputs = { self, nixpkgs, crane, flake-utils, fenix }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        # nixpkgs' `importCargoLock` hardcodes the crates.io *API* endpoint
        # (`https://crates.io/api/v1/crates`) as the download URL for the
        # crates.io registry, and crates.io answers any request whose
        # User-Agent contains `curl/<version>` with a 403 — which is exactly
        # what Nix's fetcher sends (`curl/<v> Nix/<v>`). Measured against the
        # live endpoint: `curl/8.16.0 Nix/2.31.2` -> 403, `Nix/2.31.2` -> 200,
        # and `static.crates.io` -> 200 for every UA tried, including the
        # blocked one.
        #
        # Nothing in this repo calls `importCargoLock` directly. The path that
        # bites is crane's own helper: `crane-utils` is a
        # `rustPlatform.buildRustPackage { cargoLock.lockFile = ...; }`, so its
        # dependencies — including `toml_edit 0.25.3+spec-1.1.0` — are fetched
        # through it. A cold cache therefore fails the whole `nix build` on a
        # dependency of a dependency of our build tool, with no line of our
        # code involved. This is not hypothetical: it broke `main`'s post-merge
        # run for #302 twice, once the 4G-capped store cache evicted that one
        # tarball.
        #
        # Upstream fixed it by switching the URL to the CDN (`static.crates.io`
        # on nixpkgs master, nixos-unstable and nixos-25.11); our pin predates
        # that. Rather than bump nixpkgs wholesale — which would also move
        # Flutter and its Skia, and the goldens are rasteriser-sensitive by
        # design — apply the same change surgically. `importCargoLock` merges
        # `extraRegistries` over its defaults with `//`, so passing the same
        # key replaces the URL and nothing else. Drop this overlay when the
        # nixpkgs pin advances past the upstream fix.
        # `importCargoLock` is reached through `rustPlatform.buildRustPackage`,
        # which is `callPackage`d and captured its own `importCargoLock` — so
        # overriding the `rustPlatform.importCargoLock` *attribute* does not
        # reach it (measured: 213 api/v1 URLs survived that attempt). Rewrite
        # at the fetcher instead, which every consumer goes through.
        #
        # Hash-safe by construction: the caller supplies `sha256`, and the two
        # URLs serve byte-identical tarballs (checked for the crate that broke
        # us — 66587 bytes, sha256 a0a07913…, from both). A wrong rewrite would
        # therefore fail loudly as a hash mismatch, not silently.
        # Point a fetcher's URLs at the crates.io CDN instead of the API
        ...
        cratesIoCdnOverlay = _final: prev: {
          ...
          # `fetchurl` captured in its own closure, so it does NOT inherit the
          # override below — `cargo-c` still resolved to api/v1 until this was
          # rewritten too. Both are patched through one helper.
          inherit
            (prev.lib.mapAttrs (_: redirectToCdn) {
              inherit (prev) fetchurl fetchzip;
            })
            fetchurl
            fetchzip
            ;
