...
- `PATH` gets a `cc-xcode-env` wrapper (macOS only) — run anything that drives Xcode *directly* through it: `cc-xcode-env flutter build ios --simulator`. The `xcrun` shim below only sees calls routed via `xcrun`, which is how nixpkgs' patched Flutter behaves; a stock Flutter (Homebrew, fvm, CI's Flutter action) invokes `xcodebuild` itself, so it inherits this shell's `DEVELOPER_DIR` — the Nix apple-sdk, which is not an Xcode — plus every Nix toolchain variable as a build-setting override. Nothing in the failure says so: `xcodebuild -showBuildSettings` returns nothing, `productBundleIdentifier` is null, and Flutter exits with "Application not configured for iOS", which reads as a missing `client/ios`.
